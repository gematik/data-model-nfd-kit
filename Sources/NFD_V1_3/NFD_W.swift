...
public class NFD_W {
    ...
    public class func deserialize(_ infoset: Infoset) -> NFD_W {
        
        let _NFD_W = NFD_W()
        
        _NFD_W.w = String.deserialize(infoset.attr("w"))
        _NFD_W.s = String?.deserialize(infoset.attr("s"))
        _NFD_W.gwe = String?.deserialize(infoset.attr("gwe"))
        
        return _NFD_W
    }
}

extension Optional where Wrapped == NFD_W {
    public static func deserialize(_ infoset: Infoset) -> NFD_W? {
        if infoset.count == 0 {
            return nil
        } else {
            return NFD_W.deserialize(infoset)
        }
    }
}

extension Array where Element: NFD_W {
    public var namespaceUri: String? {
        get {
            return NFD_W.namespaceUri
        }
    }
    
    public static func deserialize(_ infoset: Infoset) -> [NFD_W] {
        return infoset.map { (element: Infoset) in
        NFD_W.deserialize(element)
    }
}
}
